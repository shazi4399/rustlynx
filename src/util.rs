...
pub fn float_to_ring(x: f64, precision: usize) -> Wrapping<u64> {
    if x >= 0.0 {
        return Wrapping((x * 2f64.powf(precision as f64)) as u64);
    }

    -Wrapping(((-x) * 2f64.powf(precision as f64)) as u64)
}

pub fn ring_to_float(x: Wrapping<u64>, precision: usize) -> f64 {
    if x.0 >> 63 == 0 {
        return (x.0 as f64) / 2f64.powf(precision as f64);
    }

    -((-x).0 as f64) / 2f64.powf(precision as f64)
}
