...
Result<(Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>), Box<dyn Error>> {
    ...
    let column_major_arvs = if use_pregenerated_splits_and_selections {
        load_arvs_from_file(arv_path, ctx.num.asymm as usize, feature_count, attribute_count, tree_count)?}
    else {create_selection_vectors(fsv_amount, xtctx.tc.attribute_count, seed, ctx)?};
    ...
    let smallest_neg_value = if asym == 0 {u64::MAX} else {0}; // does this assume that Lambda = 64? Answer: Yes

    // NOTE
    // Slightly more complicated than it needs to be, artifact of past functionality.
    // We are simply slecting appropiate min/max (range) of columns
    ...
    let selected_splits: Vec<Wrapping<u64>> = if use_pregenerated_splits_and_selections
    {load_splits_from_file(splits_path, ctx.num.asymm as usize, feature_count, tree_count, decimal_precision)?}
    else {range_times_ratio.iter().zip(selected_mins.iter()).map(|(x, y)| util::truncate(*x, decimal_precision, asym) + y).collect()};
    ...
    let comparison_results: Vec<Vec<Vec<Wrapping<u64>>>> =
        cmp_res.chunks(instance_count * feature_count).map(|x| x.chunks(instance_count).map(|x| x.to_vec()).collect()).collect();
    let neg_comparison_results: Vec<Vec<Vec<Wrapping<u64>>>> =
        cmp_res_neg.chunks(instance_count * feature_count).map(|x| x.chunks(instance_count).map(|x| x.to_vec()).collect()).collect();
    ...
    //get the split points for each set of arvs - Doubled for proper ohe
