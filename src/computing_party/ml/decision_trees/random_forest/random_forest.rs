...
pub fn rf_preprocess(data: &Vec<Vec<Wrapping<u64>>>, classes: &Vec<Vec<Wrapping<u64>>>, rfctx: &mut RFContext, ctx: &mut Context)->Result<(Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>), Box<dyn Error>> {
    ...
    let (column_major_arvs_unexp, column_major_arvs) =
        create_selection_vectors_rf(fsv_amount, attribute_count, feature_count, bucket_size, seed, decision_tree, ctx)?;
    ...
    let column_major_arvs_unexp_row_maj: Vec<Vec<Vec<Wrapping<u64>>>> =
        two_dim_to_3_dim(&column_major_arvs_unexp, feature_count)?.iter().map(|x| util::transpose(&x).unwrap()).collect();
    let row_major_arvs: Vec<Vec<Vec<Wrapping<u64>>>> =
        two_dim_to_3_dim(&column_major_arvs, feature_count * bucket_size)?.iter().map(|x| util::transpose(&x).unwrap()).collect();
    ...
    let mut attribute_reduced_sets = if decision_tree
    {vec![util::transpose(&discretized_ohe_data)?]} else {batch_matmul(&discretized_ohe_data,&final_row_major_arvs, ctx)?};
    ...
    if decision_tree {
        ...
        for i in 0 .. tree_count {
            ...
            final_datasets.push(
                batch_matmul(&util::transpose
                    (&attribute_reduced_sets[i])?,&vec![util::transpose(&instance_selection[i])?],ctx)?.into_iter().flatten().collect());
        }
    }

    let final_classes: Vec<Vec<Vec<Wrapping<u64>>>> = if decision_tree
    {vec![classes.clone()]} else {batch_matmul(&classes, &instance_selection.iter().map(|x| util::transpose(&x).unwrap()).collect(), ctx)?};

    println!("final_splits");
    let final_splits = if decision_tree
    {vec![util::transpose(&full_splits)?]} else
    {batch_matmul(&util::transpose(&full_splits)?, &column_major_arvs_unexp_row_maj, ctx)?.iter().map(|x| util::transpose(&x).unwrap()).collect()};
...
pub fn create_selection_vectors_rf(quant: usize, size: usize, amount_per_tree: usize, bin_count: usize, seed: usize, decision_tree: bool, ctx: &mut Context) ->
Result<(Vec<Vec<Wrapping<u64>>>, Vec<Vec<Wrapping<u64>>>), Box<dyn Error> >{
    if ctx.num.asymm == 0 {
        if decision_tree {
