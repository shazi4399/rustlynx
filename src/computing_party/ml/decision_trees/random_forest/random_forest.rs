...
pub fn rf_preprocess(data: &Vec<Vec<Wrapping<u64>>>, classes: &Vec<Vec<Wrapping<u64>>>, rfctx: &mut RFContext, ctx: &mut Context)->Result<(Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>, Vec<Vec<Vec<Wrapping<u64>>>>), Box<dyn Error>> {
    ...
    let feature_count = rfctx.feature_count;
    let attribute_count = rfctx.tc.attribute_count;
    let instance_select_count = rfctx.instance_selected_count;
    let instance_count = rfctx.tc.instance_count;
    ...
    let mut attribute_reduced_sets = batch_matmul(&discretized_ohe_data,&final_row_major_arvs, ctx)?;
    ...
    let instance_selection_path = "custom_randomness/instance_selection.csv";
    ...
    let final_splits = batch_matmul(&util::transpose(&full_splits)?, &column_major_arvs_unexp_row_maj, ctx)?.iter().map(|x| util::transpose(&x).unwrap()).collect();
    rfctx.tc.attribute_count = bucket_size * feature_count;
...
pub fn create_selection_vectors_rf(quant: usize, size: usize, amount_per_tree: usize, bin_count: usize, seed: usize, ctx: &mut Context) -> Result<(Vec<Vec<Wrapping<u64>>>, Vec<Vec<Wrapping<u64>>>), Box<dyn Error> >{
    if ctx.num.asymm == 0 {
    ...
    for i in 0 .. quant {
        if i % amount_per_tree == 0 {
            rng.shuffle(indices.as_mut_slice());
        }
        let index = indices[i % amount_per_tree];
